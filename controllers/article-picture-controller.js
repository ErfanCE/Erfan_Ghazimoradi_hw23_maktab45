...
const articlePictureStorage = multer.diskStorage({
    destination: (request, file, cb) => {
        ...
        fs.access(path.join(__dirname, '..', 'public', 'images', 'articles', request.session.blogger.username), (err) => {
            if (err) fs.mkdir(path.join(__dirname, '..', 'public', 'images', 'articles', request.session.blogger.username), (err) => {
                if (err) return console.log('err create dir(article picture) + err.message');

                cb(null, path.join(__dirname, '..', 'public', 'images', 'articles', request.session.blogger.username));
            });
            else cb(null, path.join(__dirname, '..', 'public', 'images', 'articles', request.session.blogger.username));
        });
    }, 
